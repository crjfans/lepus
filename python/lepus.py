...
def main():
    ...
    monitor_backups = str(func.get_option('monitor_backups'))
    ...
    if monitor=="1" :
        ...
        time.sleep(3)
        if monitor_backups=="1":
            job = Process(target = job_run, args = ('check_backups',frequency_monitor))
            joblist.append(job)
            job.start()    
        ...
        time.sleep(3)
        job = Process(target = job_run, args = ('check_mysql_variables',frequency_monitor))
        joblist.append(job)
        job.start()    
