...
def get_alarm_mysql_variables():
    ...
    if result <> 0:
        for line in result:
            ...
            host=line[1]
            port=line[2]
            variable_name=line[3]
            variable_value_old=line[4]
            variable_value_new=line[5]
            remarks=line[6]
            process_user=line[7]
            create_time=line[8]
            alarm_variable_change=line[9]
            variable_monitor=line[10]
            send_mail=line[11]
            send_mail_to_list=line[12]
            send_sms=line[13]
            send_sms_to_list=line[14]
            ...
            is_delete=line[18] 
            mysql_variables_alarm_id=line[19] 
            if send_mail_to_list is None or  send_mail_to_list.strip()=='':
                send_mail_to_list = mail_to_list_common
            if send_sms_to_list is None or  send_sms_to_list.strip()=='':
                send_sms_to_list = sms_to_list_common
