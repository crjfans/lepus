...
class MySQL_model extends CI_Model{
    ...
    function get_variables_total_not_process_row(){
	$this->db->select('id');
    ...
    function get_variables_total_record(){
...
max(b.Lock_time_max) Lock_time_max, min(b.Lock_time_min) Lock_time_min,sum(b.Lock_time_sum) Lock_time_sum');
    ...
    function get_bigtable_chart_record($server_id,$db_name,$table_name,$time){
        $query=$this->db->query("select table_size from mysql_bigtable_history where server_id=$server_id and db_name='$db_name' and table_name='$table_name' and Ymd=$time order by id desc limit 1; ");
        if ($query->num_rows() > 0)
        {
           return $query->row_array(); 
        }
