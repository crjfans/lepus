...
class Servers_mysql extends Front_Controller {
    ...
    public function add(){
		...
		if(isset($_POST['submit']) && $_POST['submit']=='add')
        {
            ...
            $this->form_validation->set_rules('threshold_warning_backup_delay',  'lang:alarm_ehreshold', 'trim|required|integer');
            ...
            $this->form_validation->set_rules('threshold_critical_backup_delay',  'lang:alarm_threshold', 'trim|required|integer');
			...
			else
			{
					...
					$data = array(
                        ...
                        'alarm_backup_status'=>$this->input->post('alarm_backup_status'),
                        'alarm_backup_delay'=>$this->input->post('alarm_backup_delay'),
                        ...
                        'threshold_warning_backup_delay'=>$this->input->post('threshold_warning_backup_delay'),
              ...
              if(!empty($host) && !empty($port) && !empty($username) && !empty($password) && !empty($tags)){
                 ...
                 $data['error_code']=0;
					$data = array(
                        ...
                        'alarm_backup_status'=>$this->input->post('alarm_backup_status_'.$n),
                        'variable_monitor'=>$this->input->post('variable_monitor_'.$n),
                        'alarm_variable_change'=>$this->input->post('alarm_variable_change_'.$n),
					);
					$this->servers->insert($data);
              }
		   }
